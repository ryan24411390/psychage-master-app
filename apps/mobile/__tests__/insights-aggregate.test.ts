...
// Pure aggregation: tools with no data are omitted; the rest sort newest-used first.
...
const moment = (createdAt: string, emotions: string[]) =>
  ({ id: createdAt, date: '2026-06-15', createdAt, emotions, triggers: [] }) as never;
...
describe('buildToolSummaries', () => {
  ...
  it('surfaces the most-noted emotion for the mood journal', () => {
    const out = buildToolSummaries({
      ...EMPTY,
      mood: [moment('2026-06-10T08:00:00.000Z', ['calm', 'joy']), moment('2026-06-11T08:00:00.000Z', ['calm'])],
    });
    expect(out[0]?.metric).toBe('Most noted: calm');
  });
