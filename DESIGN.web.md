...
| Q2 | Brand teal "with opacity" expression | **Auto-resolved** | Canonical = `rgba(26, 155, 140, *)` = brand `#1A9B8C`. `rgba(13, 148, 136, *)` (= Tailwind default `#0D9488`) is wrong-teal drift. `#15B8A6` is **not** drift — per Q10 it's `color.mood.5` (mood-feature-scoped, paired L/D); visually adjacent to brand teal but a distinct token under a separate namespace, so the brand-teal opacity rule does not apply. | Ladder: surfacing rule — brand value wins over Tailwind default. Drift list in `tokens/web.tokens.json` → `color._excluded._note`. |
