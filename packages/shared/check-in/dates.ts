...
// `Date` and the `YYYY-MM-DD` string the store keys entries by.
//
// "Local" is load-bearing: a check-in saved at 00:30 belongs to that day, not to
// UTC's day. We read the local-time accessors (getFullYear/getMonth/getDate),
// never the UTC ones, so the calendar day is the DEVICE's day.

import { CheckInValidationError, type LocalCalendarDate } from './types';
 ...
 * and day (01–31). Cheap structural validation — not a full days-in-month check;
 * the only producer of these values is `toLocalCalendarDate`, which is correct by
 * construction, so this guards against hand-edited / corrupt persisted blobs.
 */
 ...
 * Use at the API boundary when a consumer (e.g. history range) hands in a string.
 */
export function asLocalCalendarDate(value: string): LocalCalendarDate {
  if (!isLocalCalendarDate(value)) {
    throw new CheckInValidationError(`"${value}" is not a YYYY-MM-DD local calendar date`);
  }
  return value;
}
