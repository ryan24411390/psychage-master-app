...
export function generateProviderQuestions({
  ...
  const severityCount = Array.from(selectedSymptoms.values()).filter(
    (s) => s.severity && s.severity >= 7,
  ).length;
  ...
  if (conditionNames.length > 1) {
    ...
    questions.push(`What treatment approaches would you recommend based on what I'm experiencing?`);
  }

  // Q4: Differential diagnosis
  if (conditionNames.length > 0) {
    questions.push(`Are there other conditions I should be aware of that have similar symptoms?`);
  }
  ...
  if (results.safety.has_crisis || results.safety.has_urgent) {
    ...
    questions.push(`What should I monitor or track before our next appointment?`);
  }
