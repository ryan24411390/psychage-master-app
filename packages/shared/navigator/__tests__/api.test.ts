...
describe("Analytics Payload Validation", () => {
  ...
  function validateAnalyticsPayload(body: Record<string, unknown>): {
    valid: boolean;
    error?: string;
  } {
...
describe("Knowledge Base Response Structure", () => {
  ...
  function validateKnowledgeBaseStructure(kb: KnowledgeBase): { valid: boolean; errors: string[] } {
    ...
    if (kb.conditions.length > 0) {
      ...
      if (!Array.isArray(c.symptom_mappings))
        errors.push("Condition missing symptom_mappings array");
    }
    ...
    if (kb.matchingConfig) {
      ...
      if (typeof mc.min_relevance_threshold !== "number")
        errors.push("min_relevance_threshold must be a number");
    ...
    if (
      kb.crisisResources &&
      !kb.crisisResources["DEFAULT"] &&
      Object.keys(kb.crisisResources).length > 0
    ) {
      ...
      conditions: [
        {
          ...
          category: "mood",
          description_for_user: "Test",
          minimum_duration: "2_weeks",
          minimum_duration_display: "2 weeks",
