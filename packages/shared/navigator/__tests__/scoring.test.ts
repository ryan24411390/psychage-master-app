...
import type { KnowledgeBase, ScoringConfig, UserSymptomInput } from "@/lib/navigator/types";
import { DEFAULT_MATCHING_CONFIG, normalizeSymptoms } from "@/lib/navigator/utils";
...
describe("Scoring Engine — Condition Matching", () => {
  ...
  describe("Mixed presentations", () => {
    it("Both MDE and GAD appear for mixed depression + anxiety inputs", () => {
      ...
      const hasAnxietyCondition = resultConditions.some((c) => c.category === "anxiety");
  ...
  describe("Edge cases", () => {
    ...
    it("No matches produces general wellness recommendations", () => {
      ...
      if (results.results.length === 0) {
        expect(results.general_recommendations.length).toBeGreaterThan(0);
        expect(
          results.general_recommendations.some(
            (r) =>
              r.toLowerCase().includes("wellness") ||
              r.toLowerCase().includes("not strongly match"),
          ),
        ).toBe(true);
      }
    });
  ...
  describe("Input validation", () => {
    ...
    it("Fractional severity rounds to nearest integer", () => {
      const inputs: UserSymptomInput[] = [{ symptom_id: "MOD_001", severity: 6.7 }];
      ...
      const inputs: UserSymptomInput[] = [
        {
          symptom_id: "MOD_001",
          severity: 5,
          duration: "BOGUS_DURATION" as any,
          frequency: "often",
        },
      ];
    ...
    it("Missing severity defaults to 5", () => {
      const inputs: UserSymptomInput[] = [{ symptom_id: "MOD_001" }];
