...
import type { EnhancedCitation, QualityGateInput } from "@/lib/article-framework/types";

// ---------------------------------------------------------------------------
// Helpers
// ---------------------------------------------------------------------------

function makeCitation(overrides: Partial<EnhancedCitation> = {}, index = 0): EnhancedCitation {
  return {
    id: `cit-${index}`,
...
describe("runQualityGate", () => {
  ...
  describe("source tier compliance", () => {
    it("fails when Tier 1 percentage is below 40%", () => {
      ...
      const citations = [
        ...Array.from({ length: 3 }, (_, i) =>
          makeCitation({ tier: 1, sourceType: "peer_reviewed_journal" }, i),
        ),
        ...Array.from({ length: 9 }, (_, i) =>
          makeCitation({ tier: 5, sourceType: "reference" }, i + 3),
        ),
      ];
      const result = runQualityGate(makeInput({ citations }));
    ...
    it("fails when Tier 1+2 combined is below 60%", () => {
      ...
      const citations = [
        ...
        ...Array.from({ length: 1 }, (_, i) =>
          makeCitation({ tier: 2, sourceType: "nimh" }, i + 5),
        ),
        ...Array.from({ length: 6 }, (_, i) =>
          makeCitation({ tier: 3, sourceType: "nami" }, i + 6),
        ),
      ];
      const result = runQualityGate(makeInput({ citations }));
      const tierCheck = result.checks.find((c) => c.id === "source_tier");
    ...
    it("passes with proper tier distribution", () => {
      ...
      const citations = [
        ...
        ...Array.from({ length: 3 }, (_, i) =>
          makeCitation({ tier: 2, sourceType: "nimh" }, i + 6),
        ),
        ...Array.from({ length: 3 }, (_, i) =>
          makeCitation({ tier: 3, sourceType: "nami" }, i + 9),
        ),
      ];
      const result = runQualityGate(makeInput({ citations }));
      const tierCheck = result.checks.find((c) => c.id === "source_tier");
  ...
  describe("citation recency", () => {
    it("warns when too many old citations", () => {
      const citations = Array.from({ length: 12 }, (_, i) => makeCitation({ year: 2010 }, i));
      const result = runQualityGate(makeInput({ citations }));
    ...
    it("passes when citations are recent", () => {
      const citations = Array.from({ length: 12 }, (_, i) => makeCitation({ year: 2024 }, i));
      const result = runQualityGate(makeInput({ citations }));
      const recencyCheck = result.checks.find((c) => c.id === "citation_recency");
